...
<body>
    ...
    <div class="nxl-content">
                    ...
                    <div class="page-header-right ms-auto">
                        <div class="page-header-right-items">
                            ...
                            <a href="add-accessories.php" class="btn btn-md btn-primary">
                                    <i class="feather-plus me-2"></i>
                                    <span>Accessories Without Serial Numbers</span>
                                </a>

                                <a href="add-accessories-2.php" class="btn btn-md  btn-light">
                                    <i class="feather-plus me-2"></i>
    ...
    <div class="card mb-0">
    <div class="card-body">
    <form action="assets/php/form-helper/addaccessories2.php" id="addaccessoriesForm" method="post" class="form-container pt-2">
    <div class="row">
        <!-- Supplier Details -->
        <div class="col-md-5">
        ...
        <div class="col-md-6">
            ...
            <div class="row">    
                <div class="col-12">
                    <div class="mb-5">
                        <label>Serial Numbers<span class="text-danger">*</span></label>
                        <textarea name="serial_numbers" class="form-control" rows="10" required style="height: 187px;"></textarea>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('#addaccessoriesForm').on('submit', function(event) {
        ...
        $.ajax({
            url: 'assets/php/form-helper/addaccessories2.php', // The PHP script that handles the form submission
            type: 'POST',
            data: formData,
            dataType: 'json',
            success: function(response) {
            ...
            error: function(xhr, status, error) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'An error occurred while adding the accessory.',
                    toast: true,
                    position: 'top-end',
                    showConfirmButton: false,
                    timer: 3000,
    ...
    function fetchSupplierDetails(companyName, supplierName, formId) {
        $.ajax({
            ...
            success: function(data) {
                if (data && !data.error) {
                    ...
                    if (data.mobile_number) {
                        ...
                        $('#mobile_number').val('');  // Clear if no data
                    }

                    if (data.address) {
                        $('#address').val(data.address);
                    } else {
                        $('#address').val('');  // Clear if no data
                    }
                }
            },
            error: function(xhr, status, error) {
    ...
    $('#bill_no').on('input', function() {
        ...
        if (billNo) {
            $.ajax({
                ...
                success: function(data) {
                    if (data && !data.error) {
                        ...
                        // Clear fields if Bill No not found
                        $('#bill_amount').val('');
            ...
            // Clear fields if Bill No is empty
            $('#bill_amount').val('');
            $('#company_name').val(null).trigger('change');
            $('#supplier_name').val(null).trigger('change');
            $('#mobile_number').val('');
