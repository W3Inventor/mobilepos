...
.mr-0{
    ...
    .search-container {
    position: relative; /* This should be the container of the search bar */
}
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="search-container">
                    <form>
                        <input class="form-control mb-2" name="sarch" id="search" placeholder="Search...">
                    </form>
                    <ul id="search-results" class="list-group" style="display:none; position:absolute; z-index:1000; max-height:200px; overflow-y:auto;"></ul>
                </div>
            </div>
            <div class="card-body">
...
$(document).ready(function () {
    ...
    $('#search').on('keyup', function () {
        ...
        if (query.length > 2) {
            $.ajax({
                url: 'assets/php/helper/payment-helper/search.php',
                method: 'POST',
    ...
    $(document).on('click', '.search-result-item', function () {
        var selectedProduct = $(this).data('id'); // This should be the IMEI or appropriate product ID
        var productType = $(this).data('type'); // Ensure this is correctly identifying 'mobile' or 'accessory'
        var itemName = $(this).text(); // Capture the exact name from the search result

        // Fetch product details using AJAX
        $.ajax({
            url: 'assets/php/helper/payment-helper/get-product-details.php',
            method: 'POST',
            data: {
            ...
            success: function (data) {
                if (data.error) {
                    ...
                    // Handle the prices dropdown
                    $('#price').empty(); // Clear existing options
                    $.each(data.prices, function (index, price) {
                        $('#price').append(`<option value="${price}">LKR ${parseFloat(price).toFixed(2)}</option>`);
                    });

                    // Populate the warranty dropdown
                    $('#warranty').empty();
                    $.each(data.warranties, function (index, warranty) {
                        $('#warranty').append(`<option value="${warranty.id}">${warranty.warranty}</option>`);
                    });

                    // Store the serial number and item type correctly
                    $('#productForm').data('serial_number', data.serial_number || '');
                    $('#productModal').data('type', productType); // Set type for the modal
                    $('#productModal').data('imei', productType === 'mobile' ? selectedProduct : ''); // Set IMEI if mobile
    ...
    // Function to handle adding the product to the table
    function saveProductToTable(itemName) {
        ...
        const warrantyText = $('#warranty option:selected').text();
        const quantity = 1; // Default quantity

        // Get item type and IMEI from the modal
        let itemType = $('#productModal').data('type') || 'accessory';
        let imei = itemType === 'mobile' ? productId : ''; // Assign IMEI only for mobile items

        // Extract the serial number from the item name using a regex pattern
        const serialNumberMatch = itemName.match(/\((\d+)\)$/); // Look for number inside parentheses at the end
        ...
        const existingRow = $('#proposalList tbody tr').filter(function () {
            return $(this).data('id') == productId &&
                $(this).data('discount') == discount &&
                $(this).data('warranty') == warranty &&
                $(this).find('.item').text().trim() === itemName;
        });

        if (existingRow.length > 0) {
            // Update existing item quantity and total
            const existingQuantity = parseInt(existingRow.find('.quantity').text());
            const newQuantity = existingQuantity + 1;
            existingRow.find('.quantity').text(newQuantity);
            existingRow.find('.total').text("LKR " + ((price - discount) * newQuantity).toFixed(2));
        } else {
            // Add a new item to the cart
            const newRow = `
                <tr data-id="${productId}" data-discount="${discount}" 
                    data-warranty="${warranty}" data-type="${itemType}" 
                    data-imei="${imei}" data-serial_number="${serialNumber}">
                    <td>${$('#proposalList tbody tr').length + 1}</td>
                    <td class="item">${itemName}</td>
                    <td class="quantity">${quantity}</td>
                    <td>LKR ${price.toFixed(2)}</td>
                    <td>LKR ${discount.toFixed(2)}</td>
                    <td>${warrantyText}</td>
                    <td class="total">LKR ${total.toFixed(2)}</td>
                    <td><button class="btn btn-link text-danger delete-btn">
                        <i class="feather-trash-2"></i></button></td>
                </tr>
            `;
            $('#proposalList tbody').append(newRow);
        }
